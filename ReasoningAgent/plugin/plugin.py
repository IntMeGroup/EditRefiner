...
class ReasoningReward(ORM):
    ...
    def compute_similarity(self, pred, gt):
        emb1 = self.get_embedding(pred)
        emb2 = self.get_embedding(gt)
        sim = F.cosine_similarity(emb1, emb2).item()
        return max(sim, 0.0)  # clip

    # ===== 主函数 =====
    def __call__(self, completions, response, **kwargs):

        rewards = []

        for pred, gt_json in zip(completions, response):

            # ===== 1. Format Reward =====
            format_score = 0.0
            if "flaw_type" in pred and "description" in pred:
                format_score = 1.0

            # ===== 解析 prediction =====
            try:
                pred_list = json.loads(pred)
                pred_item = pred_list[0]
                pred_type = pred_item.get("flaw_type", "").lower()
            ...
            # ===== 解析 GT =====
            try:
                gt_list = json.loads(gt_json)
                gt_item = gt_list[0]
                gt_type = gt_item.get("flaw_type", "").lower()
                gt_desc = gt_item.get("description", "")
            except:
                gt_type = ""
                gt_desc = ""

            # ===== 2. flaw type accuracy =====
            type_score = 1.0 if pred_type == gt_type and pred_type != "" else 0.0

            # ===== 3. semantic similarity =====
            if pred_desc and gt_desc:
                sim_score = self.compute_similarity(pred_desc, gt_desc)
            else:
                sim_score = 0.0

            # ===== final =====
            final = 0.2 * format_score + 0.4 * type_score + 0.4 * sim_score
...
# 注册
orms["reasoning_reward"] = ReasoningReward
